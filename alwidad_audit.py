...
def discover_source_links():
    ...
    # Site currently uses paginated OpenCart catalog. Scan safely beyond expected pages.
    for page in range(1, 31):
        url = with_query(SOURCE_CATALOG, page=page)
        soup = BeautifulSoup(source_get(url).text, "lxml")
        page_links = set()
...
def main():
    ...
    source_links = discover_source_links()
    if not source_links:
    ...
    for index, url in enumerate(source_links, 1):
        try:
            item = scrape_source_product(url)
